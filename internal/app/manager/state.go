...
func (state *State) AssignTaskToWorker(taskToAssign *task.Task, workerId uuid.UUID) error {
	...
	if err := state.TaskDb.Put(taskToAssign.ID.String(), taskToAssign); err != nil {return err}
	if err := state.TaskWorkerDb.Put(taskToAssign.ID.String(), &workerId); err != nil {return err}
